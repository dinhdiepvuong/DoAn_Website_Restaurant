...
function BoxSearch({ handleFilter }: Props) {
    ...
    return (
        <div className='d-flex align-items-center row box_shadow_card bg_white p-4 border_radius_3'>
            <div className='col-12 col-lg-12 d-flex mb-5 justify-content-center font_family_bold font25'>Đồ uống</div>
            <div className='mt-md-2 mt-lg-0 col-12 col-lg-12 d-flex align-items-center justify-content-center'>
                <input onChange={(e) => setSearch(e?.target?.value || '')} onKeyDown={(e: any) => e?.key === "Enter" && handleFilter(e?.target?.value)} placeholder='Nhập tên nước uống cần tìm' className='h40_px mr_10px w50_per' type="text" />
