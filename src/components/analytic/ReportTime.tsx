...
const initStateTimeActive = () => {
  const from_date = moment(new Date()).startOf('week').format('DD/MM/YYYY');
  const to_date = moment(new Date()).endOf('week').format('DD/MM/YYYY');
  const newTime = {
    title: `Tuần`,
    type: 'BY_WEEK',
...
export const ReportTime = ({ displayFilter, onChangeDate }: ReportTimeProps): JSX.Element => {
